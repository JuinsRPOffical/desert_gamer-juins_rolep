...
bot.on('ready', () => {
    ...
    if (!hideobnova){
        if (bot.guilds.find(g => g.id == "488400983496458260").channels.find(c => c.name == "info")) bot.guilds.find(g => g.id == "488400983496458260").channels.find(c => c.name == "updates-bot-user").send(`**DISCORD BOT UPDATE** @everyone\n\`\`\`diff
...
bot.on('message', async message => {
    ...
    if (message.guild.id != "455280266651369482" && message.guild.id != "488400983496458260" && message.guild.id != "495177551098937345") return
...
bot.on('raw', async event => {
    ...
    if (event.t == "MESSAGE_REACTION_ADD"){
        ...
        if (reqchannel.name == "mentions"){
            ...
            if (event_emoji_name == "➖"){
                hook(gotochannel, bot.guilds.find(g => g.id == "455280266651369482").members.find(m => m.id == event_userid).displayName, `Нет.`, requser.user.avatarURL)
                return reqchannel.fetchMessage(event_messageid).then(msg => msg.delete());
            }

            if (event_emoji_name == "➕"){
                hook(gotochannel, bot.guilds.find(g => g.id == "455280266651369482").members.find(m => m.id == event_userid).displayName, `Да.`, requser.user.avatarURL)
                return reqchannel.fetchMessage(event_messageid).then(msg => msg.delete());
            }
